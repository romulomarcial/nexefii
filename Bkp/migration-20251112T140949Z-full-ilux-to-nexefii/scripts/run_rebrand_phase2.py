...
- Produces a manifest report: manifest.json and summary printed to stdout
...
for p, original_text in candidates:
    ...
    def rep_set_lang(m):
        args = m.group(1)
        return f"try{{ localStorage.setItem('nexefii_lang',{args}); }}catch(e){{}} try{{ localStorage.setItem('ilux_lang',{args}); }}catch(e){{}}"
    ...
    for pat, repl in REPLACEMENTS:
        ...
        # Also handle PascalCase forms (Ilux -> Nexefii) and uppercase ILUX -> NEXEFII
        # direct simple replace first
        if pat.startswith('\\b'):
            # generic ilux word
