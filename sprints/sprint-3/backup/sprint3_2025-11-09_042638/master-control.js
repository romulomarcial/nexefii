...
class MasterControlSystem {
  constructor() {
    ...
    this.lang = (localStorage.getItem('nexefii_lang') || 'pt').toLowerCase();
    this.i18n = null;
    this.initSystem();
  }
  ...
  createSystemSnapshot() {
    ...
    var propertiesMap = {};
    try { propertiesMap = JSON.parse(localStorage.getItem('nexefii_properties')||'{}')||{}; } catch(e){ propertiesMap = {}; }
  ...
  el = document.getElementById('btnSaveSystemSettings'); if (el) el.addEventListener('click', () => this.handleSaveSettings());
    ...
    el = document.getElementById('btnCreateProperty'); if (el) el.addEventListener('click', () => this.openPropertyModal('create'));
  ...
  populateBackupPropertySelect() {
    try {
      ...
      if (window.NexefiiProps && typeof window.NexefiiProps.listProperties === 'function') {
        ...
        try {
          var map = JSON.parse(localStorage.getItem('nexefii_properties') || '{}');
    ...
    try {
      ...
      properties.forEach(prop => {
        ...
        if (!prop.imageUrl || prop.imageUrl === '') {
          updates.imageUrl = 'assets/hotel_default.jpg';
          needsRepair = true;
        }
  ...
  async restoreVersion(versionId) {
    ...
    try {
      ...
      // propriedades
      if (snap.properties) localStorage.setItem('nexefii_properties', JSON.stringify(snap.properties));
      // logs
  ...
  saveProperty(mode) {
    ...
    const propertyData = {
      ...
      imageUrl: propertyImage || 'assets/hotel_default.jpg',
