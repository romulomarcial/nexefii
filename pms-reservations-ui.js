...
class ReservationsUIController {
    ...
    return `
      <div class="reservation-item" data-res-id="${res.id}">
        ...
        <div class="res-actions">
          <button class="btn res-action-btn btn-edit" title="${this.t('resEdit')}">✏️</button>
          <button class="btn btn-danger res-action-btn btn-cancel" title="${this.t('resCancel')}">❌</button>
        </div>
      </div>
    `;
  }
  ...
  renderCalendar() {
    ...
    let html = `
      <div class="calendar-header">
        <button class="btn" onclick="window.reservationsUI.changeMonth(-1)">◀</button>
        ...
        <div class="calendar-day ${isToday ? 'today' : ''} ${dayReservations.length > 0 ? 'has-reservations' : ''}"
          ...
          ${dayReservations.length > 0 ? `
            <div class="day-info">
              ...
              ${checkIns > 0 ? `<span class="badge-in">↓${checkIns}</span>` : ''}
              ${checkOuts > 0 ? `<span class="badge-out">↑${checkOuts}</span>` : ''}
            </div>
          ` : ''}
        </div>
      `;
    ...
    html += `
      ...
      <div class="calendar-legend">
        <span><span class="legend-badge badge-in">↓</span> Check-in</span>
        <span><span class="legend-badge badge-out">↑</span> Check-out</span>
        <span><span class="legend-badge today-marker">●</span> Hoje</span>
      </div>
    `;
  ...
  showDayReservations(date) {
    ...
    const details = reservations.map(r => {
      ...
      let status = '';
      if (isCheckIn) status = '↓ Check-in';
      if (isCheckOut) status = '↑ Check-out';
      if (isCheckIn && isCheckOut) status = '↓↑ Check-in/out';
  ...
  renderTimeline() {
    ...
    let html = `
      <div class="timeline-header">
        <h3>Timeline - ${this.formatDateDisplay(startStr)} até ${this.formatDateDisplay(endStr)}</h3>
        <div class="timeline-controls">
          <button class="btn" onclick="window.reservationsUI.shiftTimeline(-7)">◀ 7 dias</button>
    ...
    html += `
      ...
      <div class="timeline-legend">
        ...
        <span><span class="legend-bar" style="background: transparent; border: 1px solid #ddd"></span> Disponível</span>
        <span><span class="legend-badge today-marker">●</span> Hoje</span>
      </div>
      ...
      <div class="timeline-reservations">
        ...
        <div class="timeline-res-list">
          ${reservations.length === 0 ? `
            <div class="empty-state-small">Nenhuma reserva no período</div>
          ` : reservations.map(r => {
            const checkIn = new Date(r.checkInDate);
            ...
            return `
              <div class="timeline-res-item" data-res-id="${r.id}">
                ...
                <div class="res-details">
                  ...
                  ${this.formatDateDisplay(r.checkInDate)} → ${this.formatDateDisplay(r.checkOutDate)}
  ...
  checkAvailabilityForNewRes() {
    ...
    if (!checkInDate || !checkOutDate || !roomTypeId) {
      resultDiv.innerHTML = `<span style="color: #f59e0b;">⚠️ ${this.t('resRequiredField')}</span>`;
      return;
    }

    // Check dates validity
    if (new Date(checkOutDate) <= new Date(checkInDate)) {
      resultDiv.innerHTML = `<span style="color: #ef4444;">❌ ${this.t('resCheckOutBeforeCheckIn')}</span>`;
      return;
    }

    resultDiv.innerHTML = `<span style="color: #64748b;">⏳ ${this.t('resAvailabilityCheck')}</span>`;
    ...
    setTimeout(() => {
      ...
      if (availability.available > 0) {
        resultDiv.innerHTML = `<span style="color: #10b981;">✅ ${this.t('resAvailabilityYes')} (${availability.available} ${this.t('resAvailableRooms')})</span>`;
      } else {
        resultDiv.innerHTML = `<span style="color: #ef4444;">❌ ${this.t('resAvailabilityNo')}</span>`;
      }
    }, 500);
  }
  ...
  openEditReservation(resId) {
    ...
    const saveBtn = document.querySelector('#newReservationModal .form-actions .btn.btn-primary');
    if (saveBtn) saveBtn.textContent = this.t('resBtnSaveChanges') || 'Salvar Alterações';
  ...
  saveEditedReservation() {
    ...
    if (!avail.available) {
      alert(this.t('resAvailabilityNo') || 'Sem disponibilidade para o período/tipo de quarto');
      return;
    }
    ...
    try {
      ...
      alert(this.t('resSaveError') || 'Erro ao salvar alterações');
    }
  }
  ...
  exportReservations() {
    ...
    // Create CSV
    const headers = ['Confirmação', 'Hóspede', 'Email', 'Check-in', 'Check-out', 'Noites', 'Quarto', 'Status', 'Total'];
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (sampleDataDisabled) {
    console.log('[Reservations] ⚠️  Sample data generation DISABLED (Production Mode)');
    console.log('[Reservations] To re-enable: PMSDataManager.enableSampleDataGeneration()');
  } else {
    console.log('[Reservations] 📊 Sample data generation ENABLED (Demo Mode)');
...
function generateSampleReservations(engine) {
  ...
  const today = new Date();
  const names = ['João Silva', 'Maria Santos', 'Carlos Oliveira', 'Ana Costa', 'Pedro Almeida', 'Juliana Souza', 'Roberto Lima', 'Fernanda Rocha', 'Lucas Ferreira', 'Patricia Mendes'];
