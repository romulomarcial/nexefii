...
function mapSuiteToAlertsRow(s){
  ...
  const translateAlert = (alertType) => {
    ...
    if(alertType === 'HVAC Maintenance') {
      return lang === 'en' ? 'HVAC Maintenance' : (lang === 'es' ? 'Mantenimiento HVAC' : 'Manutenção HVAC');
    }
  ...
  const thermostatStatus = (s.alerts && s.alerts.includes('Thermostat Offline')) ? 
    ...
    (lang === 'en' ? 'Online' : (lang === 'es' ? 'En línea' : 'Online'));
...
function alertsUpdateFloorCounts(){
  try{
    ...
    box.innerHTML = floors.map(f => `<span class="count-chip">${f}º: ${counts[f]}</span>`).join('');
...
function alertsApplyTexts(S, langParam) {
    ...
    const title = (langParam==='en' ? 'iLuxSys · Manage Alerts' : (langParam==='es' ? 'iLuxSys · Gestionar Alertas' : 'iLuxSys · Gerenciar Alertas'));
    ...
    set('col-lastupdate', (S && S.colLastUpdate) ? S.colLastUpdate : (langParam==='en'?'LAST UPDATE':(langParam==='es'?'ÚLTIMA ACTUALIZACIÓN':'ÚLTIMA ATUALIZAÇÃO')));
    ...
    const oMaint = document.getElementById('opt-type-maintenance'); if(oMaint) oMaint.textContent = (S && S.filterHVACMaint) ? S.filterHVACMaint : (langParam==='en'?'HVAC Maintenance':(langParam==='es'?'Mantenimiento HVAC':'Manutenção HVAC'));
...
function alertsExportCSV(){
  try{
    ...
    const headers = [
      ...
      (S && S.colLastUpdate) ? S.colLastUpdate : (lang==='en'?'LAST UPDATE':(lang==='es'?'ÚLTIMA ACTUALIZACIÓN':'ÚLTIMA ATUALIZAÇÃO'))
    ];
